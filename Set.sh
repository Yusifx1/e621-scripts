...
count=1
trap "rm url" EXIT
name=$(curl -# -A "e621downloader/1.6 (Yusifx1/e621-scripts)" "https://e621.net/post_sets/$1.json" | jq -r '.name') 
...
while [ $lc = 320 ] 
do
curl -s -A "e621-downloader/1.6 (Yusifx1/e621-scripts)"  "https://e621.net/posts.json?tags=set%3A$1+order%3Aid&limit=320&$log&$api&page=$p" |jq -r '.posts[]|.file.url' >>url
l=$(wc -l url | cut -f1 -d' ') 
lc=$((l/p))
p=$((p+1))
done
